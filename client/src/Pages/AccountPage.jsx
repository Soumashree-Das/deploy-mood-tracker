  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const userResponse = await fetch('https://deploy-mood-tracker.onrender.com/user/profile', {
          headers: {
        ...
        const moodDataResponse = await fetch(`https://deploy-mood-tracker.onrender.com/journal/mood-data?range=${timeRange}`, {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
        ...
        if (moodDataResult.length >= 7) {
          ...
          const moodFrequencyResponse = await fetch(`https://deploy-mood-tracker.onrender.com/journal/mood-frequency?range=${timeRange}`, {
            headers: {
              'Authorization': `Bearer ${token}`
            }
          });
